...
4. Now cluster resource needs to be removed from `cnpg-platform` state. Remove helm release secret for `cnpg-platform` that has the cluster resource. Then remove pg cluster from configuration of `cnpg-platform` and let installation go through. **If step 1. is not done, you will lose the pg cluster during this operation.**

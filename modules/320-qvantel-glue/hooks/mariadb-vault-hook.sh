...
hook::trigger() {
  ...
  mapfile -t clusters < <(common::get_values_value '.qvantelGlue.dbs.mariadb' | jq -r 'keys[]')
  if [[ ${#clusters[@]} -eq 0 ]]; then
    qlog "No MariaDB clusters found in values"
    exit 0
  fi
  ...
  for cluster in "${clusters[@]}"; do
    ...
    # Add per-cluster key
    jq --arg key "${cluster}-root-password" --arg val "$password" \
  ...
  # Push to Vault
  curl::execute "-d @$credentials_new --header 'X-Vault-Token: $token' '$VAULT_ADDR/v1/platform-secret/data/mariadb'" 200

  rm -f "$credentials_old" "$credentials_add" "$credentials_new" "$tmp_file"
  qlog "Vault update completed for MariaDB clusters: ${clusters[*]}"
}

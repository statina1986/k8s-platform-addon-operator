...
hook::trigger() {
  ...
  qlog "Inserting Postgres credentials for all clusters to platform-secret KV2 Vault"

  token="$(vault::get_vault_token)"

  # Get all cluster names from values
  mapfile -t clusters < <(common::get_values_value '.qvantelGlue.dbs.postgres' | jq -r 'keys[]')
  if [[ ${#clusters[@]} -eq 0 ]]; then
    qlog "No Postgres clusters found in values"
    exit 0
  fi
  ...
  curl::execute "--header 'X-Vault-Token: $token' '$VAULT_ADDR/v1/platform-secret/data/postgres'" '200|404'
  ...
  # Push to Vault
  curl::execute "-d @$credentials_new --header 'X-Vault-Token: $token' '$VAULT_ADDR/v1/platform-secret/data/postgres'" 200

  rm -f "$credentials_old" "$credentials_add" "$credentials_new" "$tmp_file"
  qlog "Vault update completed for Postgres clusters: ${clusters[*]}"
}

...
## Known Limitations
- Zalando Postgres Operator doesn't play well with Kyverno artifactory image replacement policy (see [kyverno-platform](../010-kyverno-platform/README.md)). When image in the pod is different from image in the Stateful Set, postgres operator will restart cluster pods on every reconciliation (every 30 min by default). In order to avoid we need to always set `dockerImage` to some Artifactory-based image. 

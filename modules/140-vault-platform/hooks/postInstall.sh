...
hook::trigger() {
  ...
  if [ ! "$(jq 'has("kubernetes/")' $CURL_RESULT)" == "true" ]; then
    kubectl exec -n $ADDON_OPERATOR_NAMESPACE $VAULT_RELEASE_NAME-0 -- /bin/sh -c "vault login -no-print $token && \
        ...
        vault write auth/kubernetes/config kubernetes_host='https://$KUBERNETES_SERVICE_HOST:$KUBERNETES_SERVICE_PORT' use_annotations_as_alias_metadata=true"  
  else
  ...
  qlog "Ensuring kv2 generic secrets engine enabled"
  curl::execute "--header 'X-Vault-Token: $token' '$VAULT_ADDR/v1/sys/mounts'" 200
  if [ ! "$(jq 'has("secret-v2/")' $CURL_RESULT)" == "true" ]; then
    kubectl exec -n $ADDON_OPERATOR_NAMESPACE $VAULT_RELEASE_NAME-0 -- /bin/sh -c "vault login -no-print $token && \
        vault secrets enable -path='secret-v2' kv-v2"
  else
    qlog "KV2 Generic Secrets already enabled"
  fi

  qlog "Ensuring kv2 platform secrets engine enabled"
  curl::execute "--header 'X-Vault-Token: $token' '$VAULT_ADDR/v1/sys/mounts'" 200
  if [ ! "$(jq 'has("platform-secret/")' $CURL_RESULT)" == "true" ]; then
    kubectl exec -n $ADDON_OPERATOR_NAMESPACE $VAULT_RELEASE_NAME-0 -- /bin/sh -c "vault login -no-print $token && \
        vault secrets enable -path='platform-secret' kv-v2"
  else
    qlog "KV2 Platform Secrets already enabled"
  fi

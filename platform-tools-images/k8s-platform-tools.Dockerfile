...
RUN wget https://storage.googleapis.com/kubernetes-release/release/v1.19.11/bin/linux/amd64/kubectl -O /bin/kubectl && \
    chmod +x /bin/kubectl && \
...
ADD Jenkins/deploy-platform.sh /kplat/deploy-platform.sh

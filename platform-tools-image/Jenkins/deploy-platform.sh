...
PLATFORM_READY=$(kubectl get dss -n platform platform-deployment -o=jsonpath="{$.status.conditions[?(@.type=='Ready')].status}") || "False"
...
do
    ...
    sleep 5
    N=$N-5
    PLATFORM_READY=$(kubectl get dss -n platform platform-deployment -o=jsonpath="{$.status.conditions[?(@.type=='Ready')].status}")
done
